		...
		if( isset($row['uploaded_images']) AND is_array($row['uploaded_images']) AND count($row['uploaded_images']) ) {
			...
			foreach ($row['uploaded_images'] as $temp_value) {
				...
				if($path_parts->thumb) {
					...
					$gallery_image[] = "<li><a href=\"{$path_parts->url}\" onclick=\"return hs.expand(this, { slideshowGroup: 'comments_image_{$row['id']}' })\" target=\"_blank\"><img src=\"{$path_parts->thumb}\"></a></li>";
					
				} else {
					
					$gallery_image[] = "<li><img src=\"{$path_parts->url}\"></li>";
